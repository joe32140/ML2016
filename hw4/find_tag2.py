...
for i in range(20):
    ...
    tag2.append(terms[order_centroids[i, 1]])
...
tagging_table = np.zeros(shape=(20000, 2, 20))
for i in range(len(dataset)):
    line = dataset[i].strip().lower().split()
    for j in range(20):
        if tag[j] in line:
            tagging_table[i, 0, j] = 1
        if tag2[j] in line:
            tagging_table[i, 1, j] = 1
...
with open(sys.argv[2], 'w+') as f:
    ...
    for i in range(test_data.shape[0]):
        a = int(test_data[i, 1])
        b = int(test_data[i, 2])
        tmp = [0 for j in range(20) if tagging_table[a, 0, j]==1 and tagging_table[b, 0, j]==1]
        tmp2 = [0 for j in range(20) if tagging_table[a, 1, j]==1 and tagging_table[b, 1, j]==1]
